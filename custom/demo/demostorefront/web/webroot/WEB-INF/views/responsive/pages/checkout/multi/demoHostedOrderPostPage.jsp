...
<template:demoPage pageTitle="${pageTitle}" hideHeaderLinks="true">
    <div class="demo-main">
        <div class="demo-content" style="padding: 4rem 0;">
            ...
            <div id="item_container_holder">
                ...
                <div class="item_container">
                    <form:form id="hostedOrderPagePostForm" name="hostedOrderPagePostForm"
                               action="${hostedOrderPageData.postUrl}" method="POST">
                        <div id="postFormItems">
                            <dl>
                                <c:forEach items="${hostedOrderPageData.parameters}" var="entry" varStatus="status">
                                    <c:choose>
                                        <c:when test="${hopDebugMode}">
                                            <dt><label for="${fn:escapeXml(entry.key)}" class="required"><c:out
                                                    value="${entry.key}"/></label></dt>
                                            <dd><input type="text" id="${fn:escapeXml(entry.key)}"
                                                       name="${fn:escapeXml(entry.key)}"
                                                       value="${fn:escapeXml(entry.value)}"
                                                       tabindex="${status.count + 1}"/></dd>
                                        </c:when>
                                        <c:otherwise>
                        ...
                        <c:if test="${hopDebugMode}">
                            <div class="rightcol">
                                <spring:theme code="checkout.multi.hostedOrderPostPage.button.submit"
                                              var="submitButtonLabelHtml"/>
                                <input id="button.submit" class="submitButtonText" type="submit"
                                       title="${submitButtonLabelHtml}" value="${submitButtonLabelHtml}"/>
                            </div>
                        </c:if>
                    </form:form>
                </div>
